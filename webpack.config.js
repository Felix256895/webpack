...
// 打包对应项目 npm_config_argv
const configs = JSON.parse(process.env.npm_config_argv).cooked;
let project = '';
if (configs) {
  configs.forEach((item) => {
    if (item.indexOf('--project.') > -1) {
      project = item.slice(10);
    }
  });
}
...
module.exports = {
  // 入口文件
  entry: `${root}/src/view/${project}/src/index.js`,
  // 输出文件
  output: {
    // publicPath: './', //生产环境
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      template: `${root}/src/view/${project}/public/index.html`,
      // 压缩html
      minify: {
        // 移除空格
  ...
  resolve: {
    alias: {
      style: path.resolve(__dirname, `${root}/src/view/${project}/src/assets/style`),
    },
    // 配置省略文件后缀名
    extensions: ['.wasm', '.mjs', '.js', '.json'],
    // webpack找 node_moules模块
